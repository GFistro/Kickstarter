...
p1 <- kickstarter_clean %>%
      ...
      geom_col() +
      theme_economist()
p1 + theme(axis.text.x = element_text(angle = 45, size = 10, vjust = 0.8))
...
p2 + theme(axis.text.x = element_text(angle = 45, size = 10, vjust = 0.8))
...
klasicni_prediction <- prediction(kick_rf_model$votes[, 2], clean_over$state_bi) #Na training data
...
senspec_rf <- performance(klasicni_prediction, "sens", "spec")
plot(senspec_rf)
...
xg_bal_gam_preds_res_co <- ifelse(xg_bal_gam_preds > 0.45, 1, 0)
auc(xg_test_lab, xg_bal_gam_preds_res_co) #S custom cutoff da slabsi AUC, 0.678
...
auc(xg_test_lab, xg_bal_gam_preds_res_tun)

xg_bal_model_gam_tun$params
xg_bal_model_gam_tun$callbacks
...
mlrover <- over_data[sample(nrow(over_data), (nrow(over_data)/4)), ]
mlrtest <- test_data[sample(nrow(test_data), (nrow(test_data)/4)), ]


traintask <- makeClassifTask(data = mlrover, target = "state_bi")
testtask <- makeClassifTask(data = mlrtest, target = "state_bi")
...
lrnxgpred <- predict(lrnxgmodel, testtask)
...
logipreds_optcutoff <- ifelse(logipreds > 0.597, 1, 0)
...
logi_bal_preds_optcutoff <- ifelse(logi_bal_preds > 0.597, 1, 0)
...
auc(test_data$state_bi, as.numeric(kick_rf_probs) - 1) #Navadni RF, unbalanced da AUC 0.5994
auc(xg_test_lab, xg_bal_gam_preds_res) #GBM model da AUC 0.6476
auc(test_data$state_bi, as.numeric(logipreds2)) #Navadni glm da AUC 0.5658
auc(test_data$state_bi, as.numeric(logi_bal_preds2)) #Na balanced datasetu da glm AUC 0.618
auc(test_data$state_bi, as.numeric(logi_bal_preds_optcutoff)) #Na balnced setu z optimalnim cutoffom da AUC 0.5929
auc(lrnxgpred$data$response, as.numeric(mlrtest$state_bi)) #Da AUC 0.639 glede na learner random
auc(xg_test_lab, xg_bal_gam_preds_res_tun) #Iz random learnerja vstavljeni parametri v original xgboost, da AUC 0.651
